...
function StatusIcon({ baseStatus, size = 12 }) {
  ...
  } else if (baseStatus === 'shared_value') {
...
function SourceIcon({ source, size = 10 }) {
  ...
  } else if (normalizedSource === 'Facebook') {
...
export function SourceBadge({ source }) {
  ...
  const normalizedSource = normalizeSource(source)
  const color = SOURCE_COLORS[normalizedSource] || '#6b7280'
  ...
  return (
    <span className="custom-source-badge" style={badgeStyle}>
      <SourceIcon source={normalizedSource} size={10} />
      {SOURCE_LABELS[normalizedSource] || normalizedSource}
    </span>
  )
}
